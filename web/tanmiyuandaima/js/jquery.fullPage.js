	...
	$.fn.fullpage = function(options) {
		...
		*/
		...
		$.fn.fullpage.moveSectionUp = function(){
			...
			if (prev.length) {
				scrollPage(prev, function(){
					controlFun(parseInt(prev.index()));
				}, true);
			}
		};
		...
		$.fn.fullpage.moveSectionDown = function (){
			...
			if(next.length){
				scrollPage(next, function(){
					controlFun(parseInt(next.index())); //开启当前屏幕动画
				}, false);
			}
		};
		...
		*/
		var tagcur = false, //记录第二屏幕的操作，是否释放动画
		    opactime=null;
		$(document).on('click touchstart', '#fp-nav a', function(e){
			e.preventDefault();
			var index = $(this).parent().index();
			scrollPage($('.fp-section').eq(index),function(){
				controlFun(index);
			});
		});
		controlFun(0);
		//控制每屏幕动画
        function controlFun(index)
		{
			var lines = $("#lines"),
				linespans,
				clasReg = /op01|op04|op06/g,
				opacs = ["op01","op04","op06"];
			lines.removeClass('downlines');
			lines.css({"opacity":0});
			//第一屏幕
			if(parseInt(index)==0)
			{
				clearInterval($("#xingtop")[0].t);
				clearInterval($("#xingbottom")[0].t);
				clearInterval(opactime);
				opactime = $("#xingtop")[0].t = $("#xingbottom")[0].t = null;
				$("#xingtop")[0].getContext('2d').clearRect(0,0,$(window).width(),60);
				$("#xingbottom")[0].getContext('2d').clearRect(0,0,$(window).width(),60);
				$("#Bluemix_canvas").star_back_3d({
					window_width: $(window).width(),
					window_height: $(window).height(),
					window_background: 'transparent',
					star_count: '400',
					star_color: ["#666874","128376","#257798","595b67","0d5451","73757e","#06d9c6","168579"],
					star_depth: '300',
					imgobj:{
						path:"../tanmiyuandaima/images/guang.jpg",
						x:100,
						y:100
					}
				});
			}else if(parseInt(index)==1){    //第二屏幕

				clearInterval($("#Bluemix_canvas")[0].loop);
				$("#Bluemix_canvas")[0].loop = null;
				clearInterval(opactime);
				opactime = null;
				$("#Bluemix_canvas")[0].loop = null;
				$("#Bluemix_canvas")[0].getContext('2d').clearRect(0,0,$(window).width(),$(window).height());
				$(".btn_open").off('click');
				$(".fanhui").off('click');
				$(".btn_open").click(function(){
					$(".layerB").fadeOut(function(){
						$(".dati").html('<img src="images/play.png">').prev().fadeIn();
						$(".sec02main ul li").css('margin-top','0');
					});
					$(".sec02main").fadeOut(function(){
						tagcur = true;
						lines.removeClass('downlines');
						lines.css({"opacity":0});
						clearInterval(opactime);
						opactime = null;
						$("#xingtop").star_back_points({
							counts:200,  //随机多少点
							time:30,     //定时器时间
							rangeHeight:60, //动画延展位置
							rangeHeightDiff:40,//动画差值高度位置
							rangeWidthDiff:100,//动画差值宽度位置
							pro:1,     //动画速度系数比例
							directMove:'down' //下落方向
						});
						$("#xingbottom").star_back_points({
							counts:200,  //随机多少点
							time:1000,     //定时器时间
							rangeHeight:60, //动画延展位置
							rangeHeightDiff:40,//动画差值高度位置
				...
				$(".fanhui").click(function(){
					...
					$("#xingtop")[0].t = null;
					$("#xingbottom")[0].t = null;
					$(".sec02main").fadeIn(function(){
						lines.addClass('downlines');
						animatelines();
					});
				});
                //动画的开启
				...
				clearInterval($("#Bluemix_canvas")[0].loop);
				clearInterval(opactime);
				opactime = $("#xingtop")[0].t = $("#xingbottom")[0].t = $("#Bluemix_canvas")[0].loop = null;
				$("#Bluemix_canvas")[0].getContext('2d').clearRect(0,0,$(window).width(),$(window).height());
				$("#xingtop")[0].getContext('2d').clearRect(0,0,$(window).width(),60);
				$("#xingbottom")[0].getContext('2d').clearRect(0,0,$(window).width(),60);
			}
			...
			{
				lines.stop().animate({"opacity":1},'slow',function(){
					opactime=setInterval(function(){
						linespans.each(function(i,obj){
							var rand = Math.floor(Math.random()*3);
							var clas = $(obj).attr('class');
							if(clasReg.test(clas))
							{
								var findc = clas.match(clasReg);
								$(obj).removeClass(findc[0]);
							}
							$(obj).addClass(opacs[rand]);
						});
					},1500);
				});
			}
