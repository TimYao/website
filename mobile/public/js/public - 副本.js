...
Zepto(function($){
   ...
   (function(){
     var g = (this===window) ? $(this) : $(window),
         ...
         function adderjiclick(obj,i)
         {
            ...
            subli.on('click',function(event){
              ...
              if(i==0)
              {
                 ...
                 {
                     if(a.indexOf('positionsubid')!=-1 || a.indexOf('positionsubtext')!=-1)
                     {
                         delete dataArr[a];
                     }
                 }
              ...
              //三级添加事件请求数据
              //i=0为位置数据请求，其他不需请求
              i===0 ? (reponseDate(thisDataId),subul.show(),subul.find('li').removeClass('root')) : ($(this).parent('ul').hide(),leftNav.find('li').removeClass('root'),rootIndex=0);
              addTopData();   
              event.stopPropagation();
              event.preventDefault();
              return false;
         ...
         //位置三级数据请求
         function reponseDate(thisDataId)
         {
            $.ajax({
               url:'test.php',
               type:'get',
         ...
         function addCheck()
         {
            ...
            sublibtn.on('click',function(event){
              ...
              for(var c in dataArr)
              {
                  //匹配是否已经存储了不限,存在不限点选项目替换不限存储数据
                  if(thisDataId!=-1)
                  {   
                      if(c.lastIndexOf('-1') == c.length-2)
                      {   
                          delete dataArr[c];
                          $(sublibtn).eq(0).removeClass('root').find('i').removeClass('dh');
                      }
                  }else{
                      //点选不限，清除其他同级别选项
                      if(c.indexOf('positionsubid')!=-1 || c.indexOf('positionsubtext')!=-1)
              ...
              dataArr['positionsubid'+thisDataId] = thisDataId;
              dataArr['positionsubtext'+thisDataId] = $.trim($(this).text());
              $(this).addClass('root');
              //$(this).parent().hide();
              //rightContent.children('ul').eq(0).hide();
              //leftNav.find('li').removeClass('root');
              //rootIndex = 0;
              addTopData();
                         ...
                         for(var d in dataArr)
                         {
                             ...
                             if(tag===thisFlg)
                             {
                               ...
                               {  
                                  ...
                                  if(thisId>0 && index>0)
                                  {  
                                     var kid = thisFlg+'subid'+thisId;
                                     var ktext = thisFlg+'subtext'+thisId; 
                                     delete dataArr[kid];
         ...
         function submitFun()
         {
              ...
              var lastData = [];
              if(submitBtn.length>0 && hideData.length>0)
              {
                 submitBtn.on('click',function(event){
                    hideData.val(JSON.stringify(dataArr));
      ...
      var g = (this===window) ? $(this) : $(window),
         ...
         _fun = (function(){
          ...
          function startMove(scrollcontrl)
          {
              ...
              pich = parseFloat(mainConiter.height(),10);
              $(scrollcontrl).find('li').width(w).height(pich);
   ...
   (function(){
     var g = (this===window) ? $(this) : $(window),
      ...
      dlistwrap.on('touchmove',function(){
         x1 = event.touches[0].pageX;
         y1 = event.touches[0].pageY;
         if(Math.abs(x-x1)>Math.abs(y-y1))
         {   
