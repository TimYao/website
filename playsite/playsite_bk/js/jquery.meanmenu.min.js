...
function(e) {
	...
	e.fn.meanmenu = function(n) {
		var oScreenW = $(window).width();
		var a = {
			meanMenuTarget: jQuery(this),
			meanMenuContainer: "header",
			meanMenuClose: "X",
			meanMenuCloseSize: jQuery(window).width()>=600&&jQuery(window).width()<=800?'3rem':'2rem',
			meanMenuOpen: "<span /><span /><span />",
			meanRevealPosition: "right",
			meanRevealPositionDistance: "2%",
			meanRevealColour: "",
			meanRevealHoverColour: "",
			meanScreenWidth: oScreenW,
			meanNavPush: "",
			meanShowChildren: !0,
			...
			n = e.extend(a, n),
			t = document.documentElement.clientWidth; //window.innerWidth || document.documentElement.clientWidth;
		return this.each(function() {
			...
			function i() {
				if (d >= t) {
					...
					var e = jQuery(u).html();
					var ts = jQuery('#header').offset().top+jQuery('#header').height();
					jQuery('.mean-bar').css({"minHeight":jQuery('#header').height()});
					jQuery('.mean-container .mean-nav').css({"marginTop":jQuery('.meanmenu-reveal').outerHeight()});
					jQuery(".mean-nav").html(e), C && jQuery("nav.mean-nav ul, nav.mean-nav ul *").each(function() {
						...
						e.preventDefault(), 0 == E ? (W.css("text-align", "center"), W.css("text-indent", "0"), W.css("font-size", l), jQuery(".mean-nav ul:first").slideDown(), E = !0) : (jQuery(".mean-nav ul:first").slideUp(),W.css("text-align", "center"),E = !1), W.toggleClass("meanclose"), a(), jQuery(x).addClass("mean-remove")
